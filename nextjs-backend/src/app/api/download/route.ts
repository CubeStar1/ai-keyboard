...
export async function GET() {
  ...
  try {
    ...
    if (!releaseResponse.ok) {
      ...
      return NextResponse.json({ error: 'Failed to find latest release' }, { status: releaseResponse.status })
    }
    ...
    if (!assetResponse.ok) {
      ...
      return NextResponse.json({ error: 'Failed to download asset' }, { status: assetResponse.status })
    }
